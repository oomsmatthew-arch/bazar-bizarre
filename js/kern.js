...
function bbBevat(q,...vals){ return vals.some(v=>v&&String(v).toLowerCase().indexOf(q)>=0); }

// Doorzoekt de manuals-boom (mappen + bestanden) recursief.
function bbZoekManuals(node,q,pad,out){
  if(!node) return;
  (node.folders||[]).forEach(f=>{
    if(bbBevat(q,f.name)) out.push({label:f.name,sub:'Map'+(pad.length?' in '+pad.join(' › '):'')});
    bbZoekManuals(f,q,pad.concat(f.name),out);
  });
  (node.items||[]).forEach(it=>{
    if(bbBevat(q,it.title,it.desc,it.src)) out.push({label:it.title||it.src,sub:(it.type||'pdf').toUpperCase()+(pad.length?' in '+pad.join(' › '):'')});
  });
}

// Verzamelt alle treffers, gegroepeerd per categorie.
function bbZoekAlles(q){
  ...
  voegtoe('Contacten','contacten.html', veilig(()=>B.getContacten()).filter(c=>bbBevat(q,c.naam,c.rol,c.tel,c.mail))
    .map(c=>({label:c.naam||c.rol||'Contact',sub:c.rol||c.tel||c.mail||''})));

  voegtoe('Voorraad','inventaris.html', veilig(()=>B.getPrijzen()).filter(p=>bbBevat(q,p.naam))
    .map(p=>({label:p.naam,sub:'Voorraad: '+(p.stock!=null?p.stock:'?')})));

  voegtoe('Bestellingen','bestellingen.html', veilig(()=>B.getBestellingen()).filter(b=>bbBevat(q,b.info,b.leverancier,b.cat,b.status,b.door,b.opm))
  ...
  const chk=[]; veilig(()=>B.getChecklisten()).forEach(l=>{
    if(bbBevat(q,l.naam)) chk.push({label:l.naam,sub:'Checklist'});
    (l.items||[]).forEach(it=>{ if(bbBevat(q,it.text)) chk.push({label:it.text,sub:'in '+(l.naam||'checklist')}); });
  });
  voegtoe('Checklists','checklists.html',chk);

  voegtoe('Logboek','logboek.html', veilig(()=>B.getLogboek()).filter(l=>bbBevat(q,l.tekst,l.auteur,l.datum))
  ...
  const proj=[]; veilig(()=>B.getProjecten()).forEach(p=>{
    if(bbBevat(q,p.naam,p.doel,p.status)) proj.push({label:p.naam,sub:p.doel||p.status||'Project'});
  });
  voegtoe('Projecten','projecten.html',proj);

  const man=[]; try{ bbZoekManuals(B.getManualsTree(),q,[],man); }catch(e){}
...
function bbBouwZoek(){
  ...
  ov.querySelector('#gzoekSluit').onclick=()=>bbSluitZoek();
  ov.addEventListener('click',e=>{ if(e.target===ov) bbSluitZoek(); });
  document.addEventListener('keydown',e=>{ if(e.key==='Escape'&&ov.classList.contains('open')) bbSluitZoek(); });
  bbZoekEl=ov; return ov;
}
function bbOpenZoek(){
  const ov=bbBouwZoek(); ov.classList.add('open');
...
function bbSluitZoek(){ if(bbZoekEl) bbZoekEl.classList.remove('open'); }

// Zet het vergrootglas in de balk (op elke pagina).
function bbZetZoekKnop(){
